...
; Stateless transfer for callers with an always-running upstream model.
(defmacro heat-drive-curve (input mode)
...
; Heat can suspend its scalar filter/oscillator path when drive is disabled.
; Keep execution gating separate from the transfer: a modal tensor model
; must continue advancing even when its output drive is bypassed.
(defmacro heat-drive (input mode)
  (def enabled (gt (clip (round mode) 0 6) 0))
  (def driven (block-gate enabled (heat-drive-curve input mode)))
  (selector (+ enabled 1) input driven))
