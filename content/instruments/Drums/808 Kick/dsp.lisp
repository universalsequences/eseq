; Factory ID808 — the identified SynthID TR-808 voice, rebuilt for p-locking.
;
; The body is the accepted Rung 3 identification patch (drums/synthid-808,
; 84.55% independent MR-STFT improvement); at the defaults below every hit
; reproduces the learned render. Everything added around it is a no-op at
; its default, so the instrument boots up AS the 808 and every knob is a
...
; Learned defaults: sweep endpoints 110.52541 Hz / 49.19203 Hz (ratio kept,
; the sequencer pitch supplies the endpoint), body T60 908.37796 ms
; (ampDecay -7.6044946/s), drive 1.2709932. Ranges are widened past the
...
(param start_ratio @default 2.2468154 @min 1 @max 8 @mod true @mod-mode additive @mod-depth-min -4 @mod-depth-max 4)
(param decay @default 908.37796 @min 40 @max 8000 @unit ms @mod true @mod-mode additive @mod-depth-min -2000 @mod-depth-max 2000 @mod-unit ms)
(param release @default 908.37796 @min 40 @max 8000 @unit ms @mod true @mod-mode additive @mod-depth-min -2000 @mod-depth-max 2000 @mod-unit ms)
(param drive @default 1.2709932 @min 1 @max 5 @mod true @mod-mode additive @mod-depth-min -2 @mod-depth-max 2)
(param click_amp @default 1.5 @min 0 @max 4 @mod true @mod-mode additive @mod-depth-min -2 @mod-depth-max 2)
(param noise_amp @default 0.000057686786 @min 0 @max 0.004 @mod true @mod-mode additive @mod-depth-min -0.002 @mod-depth-max 0.002)
...
(defmacro sweep-tail-params (smooth_ms)
  ; Body pitch-sweep time: the learned -51.131866/s exponential rate
  ; expressed as a T60 in ms (same convention as decay/release).
  (param sweep @default 135.09687 @min 50 @max 900 @unit ms)
...
; -> (body_amp body_asymmetry click_freq click_decay noise_cutoff noise_decay out_gain)
(defmacro body-params (smooth_ms)
  (param body_amp @default 1.0 @min 0.3 @max 1.2)
  (param body_asymmetry @default -0.093308724 @min -0.6 @max 0.6)
  (param click_freq @default 865.7889 @min 300 @max 5000 @unit Hz)
  (param click_decay @default -931.62616 @min -2500 @max -100)
  (param noise_cutoff @default 11436.708 @min 500 @max 20000 @unit Hz)
  (param noise_decay @default -0.73149204 @min -500 @max -0.001)
  (param out_gain @default 0.6676844 @min 0.3 @max 1.2)
...
(defmacro id808-voice (voice_trigger input_pitch input_velocity)
  ...
  (def mixed (+ body even_harmonic click noise_burst))
...
(defmacro bank-stage (sig triggered wet_a env_a freq_a res_a note_in)
  ...
  ; 49.19 Hz endpoint. At that reference pitch the two modes are
  ...
  (def bk_key_off (* (clip bank_track 0 1) (/ (log (/ bk_note 49.19203)) 5.586)))
...
(def voice_a (id808-voice trig_a pitch velocity))
(def voice_b (id808-voice trig_b pitch velocity))
