...
; Adds direct semitone offsets for both oscillators while preserving
; intentionally limited but musically useful modulation exposure.
...
(param filter_drive       @default 2.20  @min 0.8   @max 5 @mod true @mod-mode additive)
...
(def osc1_saw (polyblep_saw ph1 freq1))
(def osc1_pulse (polyblep_pulse ph1 pwm freq1))
(def osc2_saw (polyblep_saw ph2 freq2))
(def osc2_pulse (polyblep_pulse ph2 pwm freq2))
(def sub_pulse (polyblep_pulse phsub 0.5 sub_freq))
...
(def osc1_bright (sat (+ (* osc1 1.10) (* osc1_saw brass 0.35)) 1.08))
(def osc2_hollow (sat (+ (* osc2 0.96) (* osc2_pulse brass 0.22)) 0.96))

(def asym_mix (+ (* osc1_bright 0.64 (- 1 mix_bias))
                 (* osc2_hollow 0.86 mix_bias)
                 (* sub_pulse sub_level)
                 (* (noise) noise_level)))
(def pre_emph (+ (* asym_mix 0.78)
                 (* (sat asym_mix 1.5) 0.20)
                 (* (biquad asym_mix (clip (+ 1600 (* brass 2600)) 400 9000) 0.7 1 1) brass 0.08)))
...
(def drive_amt (mod filter_drive))
(def skew cutoff_skew)

(def predriven (sat pre_emph (+ 1.0 (* drive_amt 1.0))))
(def lp1_cut (clip (* fcut (+ 0.78 (* filter_tone 0.16) (* skew -0.10))) 30 14000))
(def lp1 (biquad predriven lp1_cut (+ 0.08 (* q 0.96)) 1 0))
(def lp2_in (+ (* lp1 0.90) (* predriven 0.20)))
(def lp2_cut (clip (* fcut (+ 0.92 (* filter_tone 0.12) (* skew 0.04))) 30 14500))
(def lp2 (biquad lp2_in lp2_cut (+ 0.22 (* q 0.88)) 1 0))
(def lp3_in (sat (+ (* lp2 0.94) (* lp1 0.12)) (+ 1.0 (* drive_amt 0.40))))
(def lp3_cut (clip (+ (* fcut (+ 1.08 (* filter_tone 0.20) (* skew 0.18))) 220) 40 15000))
(def lp3 (biquad lp3_in lp3_cut (+ 0.10 (* q 0.70)) 1 0))
(def res_air (biquad lp3 (clip (+ 2200 (* filter_tone 5200) (* q 240)) 300 16000) 0.75 1 1))
(def filt_core (+ (* lp1 0.18) (* lp2 0.48) (* lp3 0.62)))
(def filt_out (sat (+ (* filt_core 0.88)
                      (* res_air 0.10)
                      (* (sat filt_core (+ 1.0 (* brass 0.8))) 0.10 brass))
                   (+ 1.0 (* drive_amt 0.52))))
