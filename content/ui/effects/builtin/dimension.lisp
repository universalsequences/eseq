...
(export builtin-fx-dimension-ui)
...
(def option-button (fx p current label-text)
  (button label-text
...
(def color-options (fx p)
  (subtree :key (eseq.effects.builtin.filter-core/builtin-fx-param-subtree-key fx p "dimension-color")
    (let ((current (eseq.effects.param-controls/fx-param-text-value-for fx p)))
      (v-stack :gap 0.54 :align :center
        ...
        (option-button fx p current "smooth")
        (option-button fx p current "default")
        (option-button fx p current "lf sat 1")
        (option-button fx p current "lf sat 2")))))

(def shape-options (fx p)
  (subtree :key (eseq.effects.builtin.filter-core/builtin-fx-param-subtree-key fx p "dimension-shape")
    (let ((current (eseq.effects.param-controls/fx-param-text-value-for fx p)))
      (v-stack :gap 0.54 :align :center
        (label "LFO SHAPE" :font-size 8.0 :width 4.6 :color :dim :bg :transparent)
        (option-button fx p current "default")
        (option-button fx p current "sine")
        (option-button fx p current "ramp")
        (option-button fx p current "square")
        (option-button fx p current "triangle")))))

(def character-box (fx color-p shape-p)
  (box :width 11.2 :height :fill :padding 0.30
       :background-color :fx-inner-panel-bg :corner-radius 7
    (h-stack :width 5 :gap 0.40 :align :start
      (color-options fx color-p)
      (shape-options fx shape-p))))
