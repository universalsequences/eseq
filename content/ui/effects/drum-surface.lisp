...
(export panel bind value envelope strike pitch-view burst-view cut-view parameter-gesture)
(def bind (name)
  ...
  (let ((name (nth spec 0)) (title (nth spec 1)) (decimals (nth spec 2))
    ...
    (eseq.effects.custom-ui-runtime/custom-ui-param-mod-wrapper p (str "drum-mod-" name)
      (v-stack :width 8.4 :height 1.15 :gap 0.08
        ...
        (if options
          (dropdown :width 8.4 :height 0.55 :font-size 8 :options options
          ...
          (number-picker :debug-name (str "drum-detail-" name)
            :width 8.4 :height 0.55 :noui true :decimals decimals :font-size 8.5 :text-align :left
...
(def knob (spec section)
  (eseq.effects.custom-ui-lego/ui-lego-knob-styled-s section (nth spec 0) (nth spec 1)
    7.0 3.6 2.35 :cyan (nth spec 2) :linear :widget-knob-track 8.5 8 :center))
      ...
      (box :debug-name "drum-display" :width 35.8 :height 9.8 :padding 0.3 :background-color :cyan
        (v-stack :gap 0.15
          (label (str title " / " (get page :title)) :width 35.2 :height 0.7
            :h-align :center :v-align :center :font-size 8 :color :cyan :bg :black)
          (box :width 35.2 :height 4.65 ((get page :visual)))
          (box :width 35.2 :height 2.6 (details (get page :controls)))
          (h-stack :gap 0.15
            (each visible |p|
              (button (get p :title) :debug-name (str "drum-page-" (get p :id))
                :width (/ (- 35 (* 0.15 (- (len visible) 1))) (len visible))
                :height 0.8 :padding 0 :font-size 7.6
...
(defwidget eseq-drum-decay-envelope
  :width 35.2 :height 1.75 :state (duration) :bindable (duration)
  :shader
...
(def envelope (title a second-title b edit-a edit-b)
  ...
  (v-stack :gap 0.12
    (label title :height 0.4 :v-align :center :font-size 7.5 :color :black :bg :transparent)
    ...
    (label second-title :height 0.4 :v-align :center :font-size 7.5 :color :black :bg :transparent)
...
(defwidget eseq-drum-strike-position
  :width 35.2 :height 4.65 :state (sx sy) :bindable (sx sy)
  :shader
  (let ((u (/ x aspect)))
    (sdf/layer
...
(def pitch-view (base a1 a2 r1 r2 scale tune hold edit-time)
  ...
  (v-stack :gap 0.1
    ...
    (label "Drag X: time / Y: tune / 0–250 ms" :height 0.4 :v-align :center :font-size 7.3 :color :black :bg :transparent))))
...
(def burst-view ()
  ...
  (v-stack :gap 0.1
    (eseq-drum-burst-timing :debug-name "drum-bursts" :count (bind "bursts") :spread (bind "sprd")
      ...
      :on-mouse-down (get g :down) :on-drag (get g :drag) :on-mouse-up (get g :up))
    (label "Drag X: spread / Y: burst decay / 0–500 ms" :height 0.4 :v-align :center :font-size 7.3 :color :black :bg :transparent))))
(defwidget eseq-drum-layer-cut
  :width 35.2 :height 1.75 :state (length scale) :bindable (length scale)
  :shader
  (let ((u (clamp (/ (+ (/ x aspect) 0.96) 1.92) 0 1))
        (t (* 1200 u u))
        (end (* length scale))
...
(def cut-view ()
  ...
  (v-stack :gap 0.12
    (label "Body cut / 0–1.2 s" :height 0.4 :v-align :center :font-size 7.5 :color :black :bg :transparent)
    (eseq-drum-layer-cut :length (bind "length") :scale (bind "decay")
      :on-mouse-down (get ga :down) :on-drag (get ga :drag) :on-mouse-up (get ga :up))
    (label "Shell cut / 0–1.2 s" :height 0.4 :v-align :center :font-size 7.5 :color :black :bg :transparent)
...
(def quadratic-envelope (attack decay linear quadratic)
  ...
  (v-stack :gap 0.1
    (eseq-drum-quadratic-envelope :attack attack :decay decay :linear linear :quadratic quadratic
      :on-mouse-down (get g :down) :on-drag (get g :drag) :on-mouse-up (get g :up))
    (label "Drag X: decay / Y: attack / 0–500 ms" :height 0.4 :v-align :center :font-size 7.3 :color :black :bg :transparent))))
