...
impl Stream {
    /// Copy the workgroup belonging to this stream's actual AudioUnit.
    ///
    /// Call on a control thread: this takes the stream lock and queries CoreAudio.
    /// The returned non-null os_workgroup_t owns a +1 reference which the caller
    /// must release with os_release. Available on macOS 11 and later; older or
    /// unsupported output units return a backend error.
    pub fn audio_workgroup(&self) -> Result<*mut std::ffi::c_void, BackendSpecificError> {
        self.inner.lock().unwrap().audio_unit.get_property(
            self::coreaudio::sys::kAudioOutputUnitProperty_OSWorkgroup,
            Scope::Global,
            Element::Output,
        ).map_err(|error| BackendSpecificError { description: error.to_string() })
    }
