...
| Rack macros | **Second scope of the same mechanism, implemented 2026-07-17** (see `docs/racks-spec.md` A3 #6/#7 and A11). Rack macro banks are **rack-scoped with rack-relative addressing**, serialized inside the rack/rack preset — never project-global mappings pointing into a rack. They reuse the effective-send and scheduler precedence seams, expose eight stable process/p-lock targets, and map only into rack-owned slot instruments/FX/mixer params. Project-global macro-of-rack-macro mapping remains a separate follow-up. |
