...
- **grab** — a step set high plays the value from the step the source track is on right now instead of its own: the note, the velocity or the duration, whichever **value** says in the strip. It is the Cirklon inter-track grab. Set every step high and this track plays the source's melody on its own rhythm; set a few and only those steps borrow. A source track running at a slower timebase holds its step for a whole bar, so the grabbing track plays through transposed by that one note. A note grab keeps this track's own tacc offsets on top, and a chord step moves as a block so its bottom note lands on the source's. Set **value** to **note+b** instead of **note** and the grab takes the source's note as heard, with the source page's own bar transpose already in it.
- **xpose** — a step set high transposes this track's note by the note the source track is on right now, on top of the step's own transpose. It is the Cirklon "transpose by track": a source sitting on its root changes nothing, a source a fifth up moves this step a fifth up. Run the source track at a slower timebase and this track plays through under each of its notes in turn.
- **xpose+b** — the same as xpose, but it adds the source's note as you hear it, after the source page's own bar transpose. Use xpose when you want the written note and xpose+b when you want the sounding note, so a source page lifted an octave lifts this track too.
...
- grab's **source** is a track picker and **value** picks what is grabbed: note, vel, dur, or note+b.
