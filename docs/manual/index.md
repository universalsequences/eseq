...
eseq is a step sequencer with built-in instruments, effects, and a timeline. Build a looping pattern, give single notes their own sound with parameter locks, then place patterns on the arrangement or record a performance.

This edition covers the graphical workflow on macOS. Packages, scripting, and custom sequencers are not covered.

- [Concepts](concepts) — tracks, patterns, scenes, clips, and the screen
- [Your first session](first-session) — a pattern, a p-lock, an effect, a save
