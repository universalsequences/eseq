...
| `xpose+b` | gate lane | Cirklon "xpose by trk n+B" (Inter Track) | `xpose-by-track-b`: `target-add!` of `(read (track source :note+b :pattern))`, i.e. the source's current-step note *as heard*, after its own bar transpose. Appended last in `DEFAULT_LANES` (rev 7) — the list is append-only because default lane ids are index-based |
