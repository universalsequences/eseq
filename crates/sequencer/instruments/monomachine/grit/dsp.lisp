; Monomachine SID-spirit machine (grit). See docs/monomachine-family-spec.md.
...
(param osc_wave      @default 2    @min 0    @max 4)
(param pw            @default 0.5  @min 0.05 @max 0.95 @mod true @mod-mode additive)
(param tune_semi     @default 7    @min -36  @max 36 @mod true @mod-mode additive)
(param fine_cents    @default 4    @min -100 @max 100 @unit cents)
(param osc_mode      @default 0    @min 0    @max 3)
(param interlace_hz  @default 8    @min 0.5  @max 60 @unit Hz)
(param interlace     @default 0    @min 0    @max 1 @mod true @mod-mode additive)
(param bits          @default 16   @min 3    @max 16 @mod true @mod-mode additive)
(param osc2_level    @default 0    @min 0    @max 1 @mod true @mod-mode additive)

(param amp_attack_ms  @default 1    @min 0.2 @max 2000 @unit ms)
(param amp_hold_ms    @default 40   @min 0   @max 4000 @unit ms)
(param amp_decay_ms   @default 220  @min 5   @max 8000 @unit ms)
(param amp_release_ms @default 60   @min 2   @max 8000 @unit ms)
(param glide_ms       @default 0    @min 0   @max 1500 @unit ms)

(param flt_base       @default 140  @min 20  @max 11000 @unit Hz @mod true @mod-mode additive)
(param flt_width      @default 5.0  @min 0.1 @max 9 @mod true @mod-mode additive)
(param flt_res_lo     @default 2.5  @min 0.5 @max 14)
(param flt_res_hi     @default 0.8  @min 0.5 @max 14)
(param fenv_attack_ms @default 1    @min 0.2 @max 4000 @unit ms)
(param fenv_decay_ms  @default 160  @min 5   @max 8000 @unit ms)
(param env_to_base    @default 1.5  @min -6  @max 6 @mod true @mod-mode additive)
(param env_to_width   @default -1.0 @min -8  @max 8 @mod true @mod-mode additive)
...
(param srr            @default 0.12 @min 0   @max 1 @mod true @mod-mode additive)
(param eq_freq        @default 1100 @min 40  @max 11000 @unit Hz @mod true @mod-mode additive)
(param eq_gain_db     @default 0    @min -30 @max 30 @unit dB @mod true @mod-mode additive)
(param eq_q           @default 2.2  @min 0.5 @max 12)
(param drive          @default 2.0  @min 0.5 @max 8 @mod true @mod-mode additive)
(param pan_width      @default 0.1  @min 0   @max 1)
(param gain           @default 0.3  @min 0   @max 1 @mod true @mod-mode additive)

; ---- oscillators ----
(def gpitch (mnm_glide pitch glide_ms))
(def semi2 (clip (mnm_floor (mod tune_semi)) -36 36))
(def f2 (clip (* gpitch (semi_ratio (+ semi2 (/ fine_cents 100)))) 1 16000))
(def ph2 (phasor f2))
(make-history ph2_h)
(def ph2_prev (read-history ph2_h))
(def wrap2 (lt ph2 ph2_prev))
(write-history ph2_h ph2)

(def mi (clip (mnm_floor osc_mode) 0 3))
(def do_sync (max (eq mi 1) (eq mi 3)))
(def do_ring (max (eq mi 2) (eq mi 3)))

(def ph1 (phasor gpitch (* wrap2 do_sync)))
(def wi (clip (mnm_floor osc_wave) 0 4))
(def pw_ctl (clip (mod pw) 0.05 0.95))
(def o_tri (- (* 4 (abs (- ph1 0.5))) 1))
(def o_saw (polyblep_saw ph1 gpitch))
(def o_pul (polyblep_pulse ph1 pw_ctl gpitch))
(def o_mix (* o_saw o_pul))
; pitched LFSR-ish noise: latch white noise on a keytracked clock
(make-history nz_ph_h)
(make-history nz_val_h)
(def nz_prev_ph (read-history nz_ph_h))
(def nz_ph (wrap (+ nz_prev_ph (/ (* gpitch 8) samplerate)) 0 1))
(def nz_wrapped (lt nz_ph nz_prev_ph))
(write-history nz_ph_h nz_ph)
(def nz_held (gswitch nz_wrapped (noise) (read-history nz_val_h)))
(write-history nz_val_h nz_held)
(def osc1 (selector (+ wi 1) o_tri o_saw o_pul o_mix nz_held))

(def sq2 (polyblep_pulse ph2 0.5 f2))
(def ringed (gswitch do_ring (* osc1 sq2 1.4) osc1))
(def voice_mix (+ ringed (* sq2 (clip (mod osc2_level) 0 1) 0.6)))

; interlace: alternate the signal with zero at an audible-motion rate
(def il_gate (lt (phasor interlace_hz) 0.5))
(def il_amt (clip (mod interlace) 0 1))
(def interlaced (* voice_mix (- 1 (* il_amt (- 1 il_gate)))))

(def bits_ctl (clip (mod bits) 3 16))
(def src (gswitch (gte bits_ctl 15.5) interlaced (mnm_bits interlaced bits_ctl)))
