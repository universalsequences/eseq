; SynthID 808 - the accepted Rung 3 TR-808 identification patch.
;
; This is a direct DGenLisp port of Examples/SynthID/Patch.swift. The identified
; defaults below come from the passing real-audio run (84.55% independent
; MR-STFT improvement). At an end pitch of 49.19203 Hz and velocity 1.0 they
; reproduce the learned render; velocity is applied only as host-level gain.
;
; MIDI pitch defines the endpoint of the body sweep. start_ratio preserves the
; learned relationship between its endpoints while allowing the sweep depth to
; be edited independently. The gate input is deliberately not part of the
...
; The learned sweep endpoints were 110.52541 Hz and 49.19203 Hz. Their ratio is
; retained as the default while the sequencer pitch supplies the endpoint.
(param start_ratio @default 2.2468154 @min 1 @max 6)
(param pitch_decay @default -51.131866 @min -80 @max -15)

(param body_amp @default 1.0 @min 0.5 @max 1.0)
; Body release is expressed as T60 milliseconds. The learned ampDecay value of
; -7.6044946/s maps exactly to 908.37796 ms via -ln(1000)/(release*0.001).
(param release @default 908.37796 @min 100 @max 4000 @unit ms)
(param body_asymmetry @default -0.093308724 @min -0.5 @max 0.5)

(param click_freq @default 865.7889 @min 600 @max 3000 @unit Hz)
(param click_amp @default 1.5 @min 0.05 @max 1.5)
(param click_decay @default -931.62616 @min -1600 @max -200)

(param noise_cutoff @default 11436.708 @min 1000 @max 20000 @unit Hz)
(param noise_amp @default 0.000057686786 @min 0 @max 0.001)
(param noise_decay @default -0.73149204 @min -400 @max -0.001)

(param drive @default 1.2709932 @min 1 @max 3)
(param out_gain @default 0.6676844 @min 0.4 @max 1)
