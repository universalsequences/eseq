; Factory ID808TOM — the identified SynthID TR-808 low tom, rebuilt for
; p-locking in the same shape as factory/id808 and id909.
;
; The body is the 808-tom Rung 3 identification patch (drums/synthid-808-tom:
; 78.07% independent MR-STFT improvement, learned distance 0.014963, the
; direct port reproduces learned.wav to 4e-5); at the defaults below every
; hit reproduces the learned render. Everything added around it is a no-op
; at its default, so the instrument boots up AS the tom and every knob is a
; departure from truth rather than a search for it:
...
;     defaults.
;
; What the tom is, per the measurement: a near-pure sine sweeping 128 -> 93 Hz
; (a shallow 1.38 ratio against the kick's 2.25) with a uniform ~-13/s decay,
; a small odd-harmonic term, and click/noise sections that sit near zero at
; the identified defaults — they are there to be turned up.
;
...
; Learned defaults: sweep endpoints 128.40256 Hz / 92.99951 Hz (ratio kept,
; the sequencer pitch supplies the endpoint; 92.99951 Hz sounds at F#5 on
; the keyboard through the family's /8 pitch mapping), body T60 533.06269 ms
; (ampDecay -12.958617/s), drive 2.2058423. Ranges are widened past the
...
(param start_ratio @default 1.38068 @min 1 @max 8 @mod true @mod-mode additive @mod-depth-min -4 @mod-depth-max 4)
(param decay @default 533.06269 @min 40 @max 8000 @unit ms @mod true @mod-mode additive @mod-depth-min -2000 @mod-depth-max 2000 @mod-unit ms)
(param release @default 533.06269 @min 40 @max 8000 @unit ms @mod true @mod-mode additive @mod-depth-min -2000 @mod-depth-max 2000 @mod-unit ms)
(param drive @default 2.2058423 @min 1 @max 5 @mod true @mod-mode additive @mod-depth-min -2 @mod-depth-max 2)
(param click_amp @default 0.1907173 @min 0 @max 4 @mod true @mod-mode additive @mod-depth-min -2 @mod-depth-max 2)
(param noise_amp @default 0.00033538853 @min 0 @max 0.004 @mod true @mod-mode additive @mod-depth-min -0.002 @mod-depth-max 0.002)
...
(defmacro sweep-tail-params (smooth_ms)
  ; Body pitch-sweep time: the learned -25.772999/s exponential rate
  ; expressed as a T60 in ms (same convention as decay/release).
  (param sweep @default 268.02295 @min 50 @max 900 @unit ms)
...
; -> (body_amp body_asymmetry click_freq click_decay noise_cutoff noise_decay
;     out_gain body_harmonic)
(defmacro body-params (smooth_ms)
  (param body_amp @default 0.3182151 @min 0.05 @max 1.2)
  (param body_asymmetry @default 0.01700413 @min -0.6 @max 0.6)
  ; The learned odd-harmonic (3rd/5th) content — the tom's H3 sits above
  ; its H2 — editable.
  (param body_harmonic @default 0.04795937 @min -2 @max 2)
  (param click_freq @default 429.5722 @min 200 @max 5000 @unit Hz)
  (param click_decay @default -1049.2579 @min -2500 @max -100)
  (param noise_cutoff @default 1560.879 @min 500 @max 20000 @unit Hz)
  (param noise_decay @default -4.593268 @min -500 @max -0.001)
  (param out_gain @default 0.3753786 @min 0.1 @max 1.2)
...
; Exponential crossfade gain (from drums/synthid-808). first_hit bypasses
...
; top-level signals, exactly like drums/synthid-808.
; ======================================================================

(defmacro id808tom-voice (voice_trigger input_pitch input_velocity)
  ...
  ; the closed form (verified against drums/synthid-808 in the audition
  ...
  ; (sin is periodic in whole cycles; the even harmonic's 2x phase too).
  ...
  ; Learned odd-harmonic (3rd/5th, triangle-series weights) content.
  (def odd_harmonics
    (* body_harm_v
  ...
  (def learned_voice (* (tanh (* mixed drive_s)) out_gain_v))
...
(defmacro bank-stage (sig triggered wet_a env_a freq_a res_a note_in)
  ...
  ; 93.0 Hz endpoint. At that reference pitch the two modes are
  ...
  (def bk_key_off (* (clip bank_track 0 1) (/ (log (/ bk_note 92.99951)) 5.586)))
...
(def (body_amp_v body_asym_v click_freq_v click_decay_v
      noise_cutoff_v noise_decay_v out_gain_v body_harm_v)
...
(def voice_a (id808tom-voice trig_a pitch velocity))
(def voice_b (id808tom-voice trig_b pitch velocity))
