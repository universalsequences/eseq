...
/// set. LEGACY, for plain (kind-less) scripts only: the host binds it around
/// evaluating a rack-recorded `(load …)`/script source and around replaying
/// those sources on project open. Kinds never read it; an instance's owner
/// is project data (`ProjectInstance::owner`, instance-kinds spec §5), and
/// the per-module owner map that used to route `(import …)` modules to a
/// rack is gone. A plain process-wide cell rather than a VM global because
/// the UI and lisp_host def-sequencer natives share one parser but not one
/// runtime.
...
/// Run `body` with every graph `def-sequencer` it evaluates owned by
/// `group_id` (legacy plain rack scripts; see [`current_graph_owner_rack`]).
...
/// [`GraphManifest`], owned by the legacy rack scope in effect
/// ([`current_graph_owner_rack`]), else the project.
pub fn parse_graph_manifest(args: &[EValue]) -> Result<GraphManifest, String> {
    parse_graph_manifest_owned(args, current_graph_owner_rack())
}

/// [`parse_graph_manifest`] with an explicit owner. Kind templates parse
/// with `None`: an instance sets its own id and owner at publish time, so a
/// kind never depends on any ambient scope.
pub fn parse_graph_manifest_owned(
    args: &[EValue],
