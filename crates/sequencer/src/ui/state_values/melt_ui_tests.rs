...
#[test]
fn melt_context_pages_have_bound_controls_and_editable_envelopes() {
    ...
    let custom_ui_source = build_custom_instrument_ui_source_with_overlay(Some((
        ...
        "instruments/Synths/Melt/ui.lisp".to_string(),
        read_factory_source(&"instruments/Synths/Melt/ui.lisp".to_string()).unwrap(),
    )));
    let mut melt_inst = test_instrument_map();
    let dsp = read_factory_source(&format!("instruments/Synths/Melt/dsp.lisp")).unwrap();
    ...
    let params = dsp.lines().map(str::trim).filter(|line| line.starts_with("(param "))
        .enumerate().map(|(index, line)| {
            ...
            let field = format!("melt-test-{}", words[1]);
    ...
    fn visit<'a>(node: &'a eseqlisp::layout::LayoutNode, panel: &eseqlisp::layout::LayoutNode,
        ...
        if matches!(node.widget_type.as_str(), "number-picker" | "knob-number" | "dropdown" | "adsr-editor" | "response-curve-editor")
            || node.widget_type.starts_with("melt-") {
    ...
    for section in 0..6 {
        ...
        let display = find_layout_node_by_debug_name(&layout, "melt-display").unwrap();
        assert_finite_nonzero_rect(display, "context display");
        let mut controls = Vec::new();
        visit(panel, panel, &mut controls, &mut seen);
        if section < 2 {
            let env = find_layout_node_by_debug_name(display, "melt-operator-envelope").unwrap();
            for prop in ["attack", "decay", "sustain", "release"] {
                assert!(matches!(env.props.get(prop), Some(Value::ReactiveRef { .. })));
            }
            for locked in [false, true] {
                ...
                let event = Value::Map([("attack", 12.0), ("decay", 230.0), ("sustain", 0.45), ("release", 340.0)]
                    .into_iter().map(|(k,v)| (k.into(), Rc::new(RefCell::new(Value::Number(v))))).collect());
                ...
                let payload = commands.iter().find_map(|cmd| match cmd {
                ...
                let indices: std::collections::HashSet<_> = updates.iter().map(|update| {
                    let Value::Map(update) = &*update.borrow() else { panic!("update"); };
                    let Value::Number(index) = *update["param-idx"].borrow() else { panic!("index"); };
                    index as usize
                }).collect();
                assert_eq!(indices.len(), 4, "independent ADSR parameters: {commands:?}");
            }
        }
        for control in controls {
            if control.widget_type == "adsr-editor" && section >= 2 {
                let release = control.props.get("debug-name") == Some(&Value::String("melt-release".into()));
            ...
            if control.widget_type.starts_with("melt-") && control.props.contains_key("on-drag") {
                for locked in [false, true] {
                    editor.runtime_mut().register_native("seq-has-selection?", move |_args, _ctx| Ok(Value::Bool(locked)));
                    editor.drain_host_commands();
            ...
            if matches!(control.widget_type.as_str(), "knob-number" | "number-picker")
                && control.props.get("debug-name") != Some(&Value::String("melt-base-note".into())) {
    ...
    for line in dsp.lines().map(str::trim).filter(|line| line.starts_with("(param ")) {
        ...
        assert!(seen.contains(&format!("melt-test-{name}")), "unreachable parameter {name}");
    }
}
