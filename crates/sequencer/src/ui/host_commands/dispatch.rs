...
pub(crate) fn dispatch_custom_host_command(
    ...
    match name {
        ...
        n if super::resample::COMMANDS.contains(&n) => super::resample::handle(name, payload, app, editor, ctx),
        n if super::sample_import::COMMANDS.contains(&n) => super::sample_import::handle(name, payload, app, editor, ctx),
        ...
        n if super::audio_settings::COMMANDS.contains(&n) => super::audio_settings::handle(name, payload, app, editor, ctx),
...
mod tests {
    #[test]
    fn scene_bank_commands_are_registered_with_their_handler() {
        for command in [
            "create-scene-bank",
            "rename-scene-bank",
            "delete-scene-bank",
            ...
            assert!(super::super::scene_banks::COMMANDS.contains(&command));
        }
    }
