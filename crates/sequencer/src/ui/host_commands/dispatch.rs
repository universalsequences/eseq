...
/// One drained `HostCommand::Custom` command. Extracted verbatim from the
/// `metal_seq` event loop; `return` here corresponds to the old `continue`
/// targeting the drain loop (nothing follows the dispatch in that loop arm).
