...
use super::graph_payload::{GraphPayload, patch_from_payload, payload_from_patch};
...
/// Version-3 sidecars carry a serialized patch graph (spec §4.1c) that is
/// authoritative on load. Version-2 sidecars have none and take the
/// project-once migration path: they load by parsing the source and
/// materialize a payload on the next patch-editor save.
const SIDECAR_GRAPH_VERSION: u32 = 3;
...
struct LayoutSidecar {
    ...
    /// Serialized patch model (spec §4.1b). Present from v3 on; authoritative
    /// on load for authored sidecars, so the patch editor never reconstructs
    /// the graph by re-parsing the generated source.
...
/// Load the patch model straight from the sidecar's graph payload (spec
/// §4.1b), layout applied. Returns `None` when the item is not patch-authored
/// (agent- and hand-written code always opens from source, and the payload of
/// an ejected item is stale by design) or when the sidecar predates v3, which
/// takes the project-once migration path in the caller.
pub(super) fn load_authored_patch(source_path: &Path) -> Option<Patch> {
    let (status, sidecar) = match load_sidecar(source_path) {
        Ok(loaded) => loaded,
        Err(error) => {
            eprintln!(
    ...
    if status == SidecarStatus::Missing || !sidecar.authored {
        return None;
    }
    if sidecar.version < SIDECAR_GRAPH_VERSION {
        return None;
    }
    let payload = sidecar.graph.as_ref()?;
    let mut patch = patch_from_payload(payload);
    apply_sidecar(&mut patch, &sidecar, true);
    Some(patch)
}
...
/// The patch model carried by a layout JSON string, layout applied.
///
/// The macro-library actions receive the patcher's own layout payload, which
/// is authoritative (spec §4.1b); rebuilding the model by parsing the emitted
/// source instead would re-materialize every inline literal as a constant node
/// and scatter fresh, unpositioned nodes across the patch. Returns `None` for
/// pre-v3 layouts, where parsing is still the only option.
pub(super) fn patch_from_layout_json(layout: &str) -> Option<Patch> {
    let sidecar: LayoutSidecar = serde_json::from_str(layout).ok()?;
    if sidecar.version < SIDECAR_GRAPH_VERSION {
        return None;
    }
    let payload = sidecar.graph.as_ref()?;
...
pub(super) fn emitted_layout_json_with_node_map(
    ...
    // Geometry comes from `emitted_patch` (the reparse, overlaid with visible
    // positions) exactly as before, but the graph payload must come from the
    // LIVE model — serializing the reparse would launder the model straight
    // back through the lossy parse the payload exists to escape (spec §4.1b).
    // Ids are remapped through `node_id_map` so the payload agrees with the
    // canonical binding names the generator just emitted.
    let graph_patch = remapped_visible_patch(&visible, node_id_map);
    let sidecar = sidecar_from_patch_with_graph(emitted_patch, &graph_patch);
...
/// Remap a live (interaction-state) patch onto the canonical node ids the
/// generator just emitted, so a serialized payload keys nodes by the same
/// names as the generated source and the layout maps.
fn remapped_visible_patch(
    visible: &Patch,
    node_id_map: &HashMap<(String, String), String>,
) -> Patch {
    let mut remapped = visible.clone();
    remap_scope_node_ids(&mut remapped, "root", node_id_map);
    for macro_patch in &mut remapped.macros {
        let view_key = format!("macro:{}", macro_patch.name);
        remap_scope_node_ids(&mut macro_patch.patch, &view_key, node_id_map);
    }
    remapped
}

fn remap_scope_node_ids(
    patch: &mut Patch,
    view_key: &str,
    node_id_map: &HashMap<(String, String), String>,
) {
    for node in &mut patch.nodes {
        node.id = mapped_node_id(view_key, &node.id, node_id_map);
    }
    for connection in &mut patch.connections {
        connection.from_node = mapped_node_id(view_key, &connection.from_node, node_id_map);
        connection.to_node = mapped_node_id(view_key, &connection.to_node, node_id_map);
    }
}

fn sidecar_from_patch(patch: &Patch) -> LayoutSidecar {
    sidecar_from_patch_with_graph(patch, patch)
}

/// `layout_patch` supplies geometry, `graph_patch` supplies the serialized
/// model. They are the same patch everywhere except the post-generation save
/// path, where geometry is keyed off the reparsed source but the model must
/// come from live editor state.
fn sidecar_from_patch_with_graph(layout_patch: &Patch, graph_patch: &Patch) -> LayoutSidecar {
    let patch = layout_patch;
    ...
    LayoutSidecar {
        ...
        macros,
        // Every sidecar write is a patch-editor write, which is exactly the set
        // of moments the model is authoritative — so the payload is refreshed
        // unconditionally here. That also covers the two projecting paths that
        // must not leave a stale payload behind (spec §4.1b): promotion and the
        // agentic macro rematerialize both end in a sidecar write.
        graph: Some(payload_from_patch(graph_patch)),
    }
}
